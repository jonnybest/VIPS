	...
	public partial class Form1 : Form
	{
		...
		public Form1()
		{
			...
			string metapaintball = "http://paintball.fsmi.uni-karlsruhe.de/bilder", // separierbar ab 8
				...
				deviantartgallery = "http://www.deviantart.com/",
				deviantartimage = "http://gamefan84.deviantart.com/art/Aegik-commission-2-364868508",
				picasa = "https://picasaweb.google.com/Paintball.Fsmi/2006Kai?authkey=Gv1sRgCISw9Kao6s6GRQ"; // separierbar ab 10

			webBrowser1.Navigate(deviantartimage);
		}
		...
		private void Analyze()
		{
			...
			var xresult = XDocument.Parse(CleanControlChars(legacyResult.xml));
			xresult.Save("segments.xml");
			//RebuildPage(XDocument.Parse(result.xml));
			RepaintPage(xresult);
		}

		private string CleanControlChars(string p)
		{
			string res = p;

			var cleanp = from char o in p
						 where char.IsControl(o)
						 select o;

			foreach (char o in cleanp)
			{
				res = res.Replace(o, ' ');
			}
